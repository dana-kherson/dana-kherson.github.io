...
;( function( $, _undefined ) {
	...
	function usGallery( container ) {
		...
		if ( ( ( self.ajaxData.template_vars || {} ).ids || [] ).length === 0 ) {
			return;
		}
	...
	usGallery.prototype = {
		...
		getItems: function() {
			...
			if ( self.numPage === self.ajaxData.template_vars.max_num_pages ) {
			...
			$.ajax( {
				type: 'post',
				url: $us.ajaxUrl,
				data: $.extend( {}, self.ajaxData,
					{
						template_vars: JSON.stringify( self.ajaxData.template_vars ), // convert for `us_get_HTTP_POST_json()`
						num_page: self.numPage,
					}
				),
				success: ( html ) => {
					var $result = $( html ),
					...
					if ( ! $items.length || self.numPage === self.ajaxData.template_vars.max_num_pages -1 ) {
						self.$loadmore.addClass( 'hidden' );
					}
